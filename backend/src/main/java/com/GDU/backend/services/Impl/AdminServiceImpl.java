...
public class AdminServiceImpl implements AdminService {
    ...
    public DocumentResponse getDocumentThisYear() {
        try {
            ...
            DocumentResponse res = DocumentResponse.builder()
                    .getNumberOfDocuments(numberOfDocsThisYear).percentage(roundedPercentage).build();
            return res;
        } catch (Exception e) {
    ...
    public DocumentResponse getDocumentThisMonth() {
        try {
            ...
            float roundedPercentage = Math.round(percentage * 100.0f) / 100.0f;
            DocumentResponse res = DocumentResponse.builder()
                    .getNumberOfDocuments(numberOfDocsThisMonth).percentage(roundedPercentage).build();
            return res;
        } catch (Exception e) {
