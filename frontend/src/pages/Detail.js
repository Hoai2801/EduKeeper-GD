...
const Detail = () => {
  ...
  return (
      <div className={`w-full`}>
        <h2 className={`text-[28px] text-center mt-10 font-bold ${data?.scope === "public" || data?.user_upload?.staffCode === staffCode || data === null ? "hidden" : "block"}`}>
        ...
        <div
          ...
          <div className={`bg-white overflow-y-scroll h-screen rounded-lg flex flex-col max-w-[1080px] mt-5 mx-auto`}>
            ...
            <div>
              <p className='text-center text-gray-500 text-xl font-semibold mt-3 mb-10 p-5'>{htmlContent !== "Đang tải tài liệu" ? "Đã hết số trang được xem review. Để xem thêm các trang sau, vui lòng tải tài liệu để có trải nghiệm tốt nhất" : ""}</p>
            </div>
          </div>
          <CommentComponent
              data={data}
