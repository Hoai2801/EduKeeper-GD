    ...
    <div className='w-full pr-10'>
      ...
      <div className='my-5 flex gap-5 h-fit flex-wrap justify-center'>
        {documents && documents.map((item, index) => {
          if (index < limit) {
            return (
              <DocumentCard key={index} title={item.title} author={item.author} upload_date={item.upload_date} slug={item.slug} thumbnail={item.thumbnail} specialized={item.subject.specialized.specializedName} specializedSlug={item.subject.specialized.specializedSlug} />
            )
          }
        }
        )}
