...
export const Upload = () => {
    ...
    return (
        <div className={`w-full p-2`}>
            <div className={`${isLoading ? "" : "hidden"} top-0 left-0 w-full h-[100vh] fixed bg-gray-500/50 z-50 flex justify-center items-center`}>
                <div role="status" className={`z-50 flex flex-col justify-center items-center`}>
                    ...
                    <h3 className={`text-xl font-bold text-white mt-5`}>Đang đăng tài liệu, quá trình này có thể diễn ra
                        trong vài phút!</h3>
                </div>
            </div>
                        ...
                        <select id="subject"
                            ...
                            <option value={null}>Chọn môn</option>
                            {
                                listSubject && listSubject.map(subject => (
                                    <option selected={subject.id === uploadData.subject} value={subject.id}
                                            key={subject.id}>{subject.subjectName}</option>
                                ))
                            }
                        </select>
                    </div>
                        ...
                        >
                            <option value={-1}>Chọn quyền riêng tư</option>
                            {
                                scopeList.map((scope, index) => (
                                    <option selected={scope.scope === uploadData.scope} key={index}
                                            value={scope.scope}>{scope.name}</option>
                                ))
                            }
                        </select>
                    </div>
