...
const Home = () => {
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/v1/documents/most-viewed?limit=10")
      .then((res) => res?.json())
      .then((data) => setMostViewed(data));
    fetch("http://localhost:8080/api/v1/documents/most-downloaded?limit=10")
      .then((res) => res?.json())
      .then((data) => setMostDownloaded(data));
    fetch("http://localhost:8080/api/v1/documents/latest?limit=10")
      .then((res) => res?.json())
      .then((data) => {
        console.log(data);
    ...
    <div>
      <div className="bg-white rounded-lg lg:w-[1200px] w-full h-fit shadow-2xl pt-5 mt-10 flex flex-col p-10">
        ...
        <div className="flex justify-center mt-5">
          <Link
            to={`/search?order=lastest&searchTerm=`}
        ...
        <div className="ml-5 flex flex-col gap-5 items-center">
          ...
          <div className="flex justify-center">
            <Link
              to={`/search?order=mostDownloaded&searchTerm=`}
              className="text-white bg-blue-500 hover:bg-blue-300 rounded-md p-4"
            >
              Xem thêm
            </Link>
