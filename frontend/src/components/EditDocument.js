...
const EditDocument = ({
  ...
  return (
    <div
      className={`absolute bottom-0 right-0 overflow-auto w-full h-full flex justify-center align-middle ${
        isShowEdit ? "" : "hidden"
      }`}
    >
      <div
        className={`bg-white w-[50%] h-[90%] fixed top-10 border-2 border-black rounded-2xl`}
      >
        ...
        <div className="w-full p-5 flex justify-center">
          <div className="w-[80%] flex flex-col gap-5">
            ...
            <select
              ...
              id=""
              className="border border-gray-300 rounded-md p-4"
              onChange={(e) => setSelectAuthor(JSON.parse(e.target.value))}
            >
              {users &&
                users.map((author) => (
                  <option
                    ...
                    key={author.id}
                    selected={selectAuthor?.id === author.id}
