...
const SideBar = () => {
    ...
    useEffect(() => {
        if (department) {
            fetch(`http://localhost:8080/api/v1/specializes/department/${department.id}`)
        ...
        <div className='flex justify-between w-full gap-5 p-5'>
            <div className={`flex md:flex-row flex-col justify-center w-full gap-5 h-fit md:px-10`}>
                <button
                    ...
                    className={`bg-sky-500 hover:bg-sky-600 text-white py-2 px-4 rounded ${isFilterShow ? 'md:hidden' : 'md:hidden'}`}
                >
                    Lọc tài liệu
                </button>
                <div className={`md:w-[25%] w-full h-full rounded-lg shadow-lg bg-white items-center flex-col gap-5 px-5 ${isFilterShow ? 'md:flex' : 'md:flex hidden'}`}>
                    ...
                    <div className='flex gap-3 mt-5'>
                        <div
                            className={`text-center border py-2 px-3 rounded-3xl border-black hover:bg-slate-500 hover:cursor-pointer hover:text-white ${sort === 'latest' ? 'bg-slate-500 text-white' : ''}`}
                            onClick={handleSort('lastest')}
                        >
                            Mới nhất
                        </div>
                        <div
                            className={`text-center border py-2 px-3 rounded-3xl border-black hover:bg-slate-500 hover:cursor-pointer hover:text-white ${sort === 'most-viewed' ? 'bg-slate-500 text-white' : ''}`}
                            onClick={handleSort('most-viewed')}
                        >
                            Xem nhiều
                        </div>
                        <div
                            className={`text-center border py-2 px-3 rounded-3xl border-black hover:bg-slate-500 hover:cursor-pointer hover:text-white ${sort === 'most-downloaded' ? 'bg-slate-500 text-white' : ''}`}
                    ...
                    <form className='max-w-sm mx-auto'>
                        <label htmlFor='departments' className='block mb-2 text-xl text-gray-900 font-semibold'>Khoa</label>
                        <select
                            id='departments'
                            ...
                            onChange={(event) => {
                                const selectedDepartment = event.target.value ? JSON.parse(event.target.value) : null;
                    ...
                    <form className='max-w-sm mx-auto w-full'>
                        ...
                        <select
                            ...
                            className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-full'
                            onChange={(event) => {
                                const selectedSpecialies = event.target.value ? JSON.parse(event.target.value) : null;
                                setSpecialized(selectedSpecialies)
                            }}
                        >
                            <option value=''>Chọn chuyên ngành</option>
                            {specializesList.map((specialized) => (
                                <option key={specialized.id} value={JSON.stringify(specialized)}>
                    ...
                    <form className='max-w-sm mx-auto w-full'>
                        <label htmlFor='specialies' className='block mb-2 text-xl text-gray-900 font-semibold'>Chuyên môn</label>
                        <select
                            id='specialies'
                            className='bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 w-full'
                            onChange={(event) => {
                                const selectedSubject = event.target.value ? JSON.parse(event.target.value) : null;
                    ...
                    <div className='w-full pt-5 flex justify-center'>
                        <button onClick={filter} className='bg-blue-500 rounded-3xl w-[100px] h-[40px] text-center text-white text-x mb-10'>
                            Lọc
                        </button>
                    </div>
                </div>
